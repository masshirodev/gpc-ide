import type { FlowProject, FlowVariable } from '$lib/types/flow';
...
export function generateMergedFlowGpc(project: FlowProject): string {
	...
	if (hasMenu && !hasGameplay) {
		return generateFlowGpc(menuFlow);
	}
	if (!hasMenu && hasGameplay) {
	...
	const menuCode = generateFlowGpc(menuFlow!);
