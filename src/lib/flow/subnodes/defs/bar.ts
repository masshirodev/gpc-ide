...
export const barDef: SubNodeDef = {
	...
	generateGpc(config, ctx) {
		...
		if (style === 'recessed') {
			...
			lines.push(`    rect_oled(${ctx.x}, ${y}, ${w}, ${h}, 0, OLED_WHITE);`);
			lines.push(`    if(${boundVar} > 0) {`);
			lines.push(`        rect_oled(${ctx.x + 2}, ${y + 2}, ${boundVar} * ${w - 4} / 100, ${h - 4}, 1, OLED_WHITE);`);
			lines.push(`    }`);
		} else if (style === 'gradient') {
			lines.push(`    rect_oled(${ctx.x}, ${y}, ${w}, ${h}, 0, OLED_WHITE);`);
			lines.push(`    _oled_fw = ${boundVar} * ${w - 2} / 100;`);
			lines.push(`    for(_oled_py = ${y + 1}; _oled_py < ${y + h - 1}; _oled_py++) {`);
			...
			lines.push(`        rect_oled(${ctx.x + 2} + _oled_i * ${segW}, ${y + 2}, ${segW - 2}, ${h - 4}, 1, OLED_WHITE);`);
			lines.push(`    }`);
			...
			lines.push(`            rect_oled(_oled_sx, ${y}, ${stepW - 1}, ${h}, 1, OLED_WHITE);`);
			lines.push(`        } else {`);
			lines.push(`            line_oled(_oled_sx, ${y}, _oled_sx + ${stepW - 2}, ${y}, 1, OLED_WHITE);`);
			lines.push(`            line_oled(_oled_sx, ${y + h - 1}, _oled_sx + ${stepW - 2}, ${y + h - 1}, 1, OLED_WHITE);`);
			lines.push(`            line_oled(_oled_sx, ${y}, _oled_sx, ${y + h - 1}, 1, OLED_WHITE);`);
			lines.push(`            line_oled(_oled_sx + ${stepW - 2}, ${y}, _oled_sx + ${stepW - 2}, ${y + h - 1}, 1, OLED_WHITE);`);
			lines.push(`        }`);
			lines.push(`    }`);
		} else if (style === 'equalizer') {
			const bars = (config.segments as number) || 5;
			...
			lines.push(`        if(_oled_bh > 0) rect_oled(_oled_bx, ${y + h} - _oled_bh, ${barW}, _oled_bh, 1, OLED_WHITE);`);
			lines.push(`    }`);
		} else if (style === 'diagnostic') {
			const showValue = (config.showValue as boolean) !== false;
			lines.push(`    rect_oled(${ctx.x}, ${y + 2}, ${w - 20}, 8, 0, OLED_WHITE);`);
			lines.push(`    if(${boundVar} > 0) rect_oled(${ctx.x + 1}, ${y + 3}, ${boundVar} * ${w - 22} / 100, 6, 1, OLED_WHITE);`);
			if (showValue) {
				lines.push(`    PrintNumber(${boundVar}, find_digits(${boundVar}), ${ctx.x + w - 16}, ${y + 2}, OLED_FONT_SMALL);`);
			}
		} else if (style === 'bidirectional') {
			const center = Math.floor(w / 2);
			lines.push(`    rect_oled(${ctx.x}, ${y + 2}, ${w}, 8, 0, OLED_WHITE);`);
			lines.push(`    line_oled(${ctx.x + center}, ${y + 2}, ${ctx.x + center}, ${y + 9}, 1, OLED_WHITE);`);
			lines.push(`    _oled_offset = (${boundVar} - 50) * ${center - 2} / 50;`);
			lines.push(`    if(_oled_offset > 0) rect_oled(${ctx.x + center + 1}, ${y + 3}, _oled_offset, 6, 1, OLED_WHITE);`);
			lines.push(`    if(_oled_offset < 0) rect_oled(${ctx.x + center} + _oled_offset, ${y + 3}, 0 - _oled_offset, 6, 1, OLED_WHITE);`);
		}
