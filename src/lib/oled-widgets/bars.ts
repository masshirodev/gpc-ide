...
export const recessedBar: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		return [
			...
			`    rect_oled(${x}, ${y}, ${w}, ${h}, 0, OLED_WHITE);`,
			`    if(${varName} > 0) {`,
			`        rect_oled(${x + 2}, ${y + 2}, ${varName} * ${w - 4} / 100, ${h - 4}, 1, OLED_WHITE);`,
			`    }`,
		].join('\n');
	},
};
...
export const chunkyRetroBar: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		return [
			...
			`        rect_oled(${x + 2} + _oled_i * ${segW}, ${y + 2}, ${segW - 2}, ${h - 4}, 1, OLED_WHITE);`,
			`    }`,
		].join('\n');
	},
};
...
export const notchedBar: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		return [
			...
			`            rect_oled(_oled_sx, ${y}, ${stepW - 1}, ${h}, 1, OLED_WHITE);`,
			`        } else {`,
			`            line_oled(_oled_sx, ${y}, _oled_sx + ${stepW - 2}, ${y}, 1, OLED_WHITE);`,
			`            line_oled(_oled_sx, ${y + h - 1}, _oled_sx + ${stepW - 2}, ${y + h - 1}, 1, OLED_WHITE);`,
			`            line_oled(_oled_sx, ${y}, _oled_sx, ${y + h - 1}, 1, OLED_WHITE);`,
			`            line_oled(_oled_sx + ${stepW - 2}, ${y}, _oled_sx + ${stepW - 2}, ${y + h - 1}, 1, OLED_WHITE);`,
			`        }`,
			`    }`,
		].join('\n');
	},
...
export const ledStrip: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		return [
			...
			`        if(_oled_i < _oled_filled) {`,
			`            rect_oled(_oled_cx - ${r}, ${y}, ${r * 2 + 1}, ${r * 2 + 1}, 1, OLED_WHITE);`,
			`        } else {`,
...
export const diagnosticBar: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		const lines = [
			...
			`    rect_oled(${x}, ${y + 2}, ${w - 20}, 8, 0, OLED_WHITE);`,
			`    if(${varName} > 0) rect_oled(${x + 1}, ${y + 3}, ${varName} * ${w - 22} / 100, 6, 1, OLED_WHITE);`,
		];
		if (showValue) {
			lines.push(`    PrintNumber(${varName}, find_digits(${varName}), ${x + w - 16}, ${y + 2}, OLED_FONT_SMALL);`);
		}
		return lines.join('\n');
	},
};
...
export const biDiagnosticBar: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		return [
			...
			`    rect_oled(${x}, ${y + 2}, ${w}, 8, 0, OLED_WHITE);`,
			`    line_oled(${x + center}, ${y + 2}, ${x + center}, ${y + 9}, 1, OLED_WHITE);`,
			`    _oled_offset = (${varName} - 50) * ${center - 2} / 50;`,
			`    if(_oled_offset > 0) rect_oled(${x + center + 1}, ${y + 3}, _oled_offset, 6, 1, OLED_WHITE);`,
			`    if(_oled_offset < 0) rect_oled(${x + center} + _oled_offset, ${y + 3}, 0 - _oled_offset, 6, 1, OLED_WHITE);`,
		].join('\n');
	},
};
...
export const equalizerBar: OledWidgetDef = {
	...
	generateGpc(config, varName, x, y) {
		...
		return [
			...
			`        if(_oled_bh > 0) rect_oled(_oled_bx, ${y + h} - _oled_bh, ${barW}, _oled_bh, 1, OLED_WHITE);`,
			`    }`,
		].join('\n');
	},
};
