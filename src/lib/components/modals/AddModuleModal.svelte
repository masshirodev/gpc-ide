<script lang="ts">
	...
	let userModules = $derived(
		compatibleModules.filter((m) => m.is_user_module && matchesSearch(m, searchQuery))
	);
	...
	let selectedModuleDetails = $derived(modules.find((m) => m.id === selectedModule));
</script>
			...
			<div class="space-y-4 px-5 py-4" style="max-height: 60vh; overflow-y: auto;">
				{#if gameType}
					<div class="rounded bg-zinc-800/50 px-3 py-2 text-sm">
						...
						<span class="ml-2 font-medium text-emerald-400 uppercase">{gameType}</span>
				...
				{#snippet moduleButton(module: ModuleSummary)}
					<button
						class="w-full rounded border px-4 py-3 text-left transition-colors {selectedModule ===
						module.id
							...
							<div class="flex-1">
								...
								{#if module.conflicts && module.conflicts.length > 0}
									<p class="mt-1 text-xs text-amber-400">
										Conflicts: {module.conflicts.join(', ')}
									</p>
								{/if}
							</div>
				...
				<div>
					<label class="mb-2 block text-sm text-zinc-300"> Select Module </label>
					<div class="space-y-2">
						{#if userModules.length > 0}
							<div class="pb-1 text-xs font-medium tracking-wider text-zinc-500 uppercase">
								Your Modules
							</div>
				...
				{#if error}
					<div
						class="rounded border border-red-800 bg-red-900/20 px-3 py-2 text-sm whitespace-pre-wrap text-red-400"
					>
						{error}
					</div>
				{/if}
